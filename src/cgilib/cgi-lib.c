 ...
 * Version: 2.0
 * Filename: cgi-lib.c
 * Maintained By: Joel Dare (joel@joeldare.com)
 *
 * CGI-LIB was originally written by Noel V Aguilar and was released to the public domain. As
 * far as I can tell, the last change was made in June of 2000.  In February of 2004 I found
 * and fixed a small bug.  After contacting the original author, I decided to fix and
 * distribute this new version.
 *
 ...
 *
 * In documentation, when I refer to true I refer to the values of SUCCESS and when I
 * refer to false, I refer to the value of FAILURE.
 ********************************************************************************************/
...
int is_form_empty(void)
{
  ...
  if(REQUEST_METHOD != NULL)
  {
		...
		{
			return SUCCESS; /* the form is empty, no data via the QUERY STRING */
		}
  ...
  else
  {
	  ...
	  fprintf(stderr,"If you are debugging, please set your CGI\n");
		...
		if (CONTENT_LENGTH != NULL) /* make sure that we did receive data */
		  {
			...
			if ((postData = (char *)malloc(sizeof(char) * content_length + 1)) != NULL) /* allocate memory  */
...
static LIST *parse_data_record(const char *data, LIST *head)
{
  ...
  if((localData = (char *)malloc(sizeof(char) * strlen(data) + 1)) != NULL) /* allocate data that we can modify */
	{
...
static CGI_LIST parse_data_keyvalue(const char *record)
{
  ...
  tempStr = (char *)malloc(sizeof(char) * loc + 1); /* loc is the = which be replaced by the null character */
  ...
  if(loc != strlen(record)) /* first check that we did find an = sign, */
  {
	...
	tempStr = (char *)malloc(sizeof(char) * strlen(record) + 1); /* need to include the 1 for null char */
...
static char *unescape_url(const char *string) /* use const cause we don't want to modify data */
{
  ...
  count = 0;
  if((newString = (char *)malloc(sizeof(char) * (strlen(string) + 1 - (numEncodedItems * 2)))) != NULL)
	{
