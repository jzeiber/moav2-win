...
HWND InitWindow(HINSTANCE hInstance,int nCmdShow)
{
	...
	if(windowed==0) {
		...
		wr.bottom=640;
		wr.right=840;
		AdjustWindowRect(&wr,WS_VISIBLE | WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX,0);
		desk_hwnd=hWnd=CreateWindowEx(
								 0,
								 "DDCWin",
								 buf,
	...
	while (*s) {
		if (*s=='-') {
			...
			if (tolower(*s)=='t') {
				...
				s++;
				while (isspace(*s)) s++;
				n=0; while (n<84 && *s && !isspace(*s)) host_addr[n++]=*s++;
				host_addr[n]=0;
			} else if (tolower(*s)=='w') {
				s++;
				windowed=1;
...
#endif
{
	...
	mutex=CreateMutex(NULL,0,TEXT("MOAB"));
	if (mutex==NULL || GetLastError()==ERROR_ALREADY_EXISTS && strcmp(host_addr,"192.168.42.1")) {
		MessageBox(0,TEXT("Another instance of ")TEXT(MNAME)TEXT(" is already running."),TEXT("Error"),MB_OK|MB_ICONSTOP);
		return 0;
	}

	lib=LoadLibrary(TEXT("CTL3D32.DLL"));
	if (lib) {
		regxx=GetProcAddress(lib,"Ctl3dRegister");
		if (regxx) regxx(GetCurrentProcess());
		ctl3don=GetProcAddress(lib,"Ctl3dSubclassDlg");
		regxy=GetProcAddress(lib,"Ctl3dUnregister");
	} else {
		regxy=NULL;
		ctl3don=NULL;
	}
	...
	// init window after options so we don't have an empty window showing up
	if(windowed==1) {
		hwnd=InitWindow(hInstance,nCmdShow);
	}
