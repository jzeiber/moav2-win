...
void convert(HWND hwnd)
{
	...
	for (n=0; n<40000; n++) {
...
#ifdef _UNICODE
		SetDlgItemTextA(hwnd,IDC_STATUS,buf);
#else
		...
		size=xs*ys;
		dat=(unsigned short *)malloc(size*2);
		...
		lseek(hidx,n*sizeof(struct gfx_struct),SEEK_SET);
		gfx.xs=xs;
		gfx.ys=ys;
		gfx.off=offset;
		write(hidx,&gfx,sizeof(gfx));
...
void conv_init(void)
{
	...
	if (hidx==-1) {
		dd_deinit();
		save_options();
		MessageBox(desk_hwnd,TEXT("Graphics file gx00.idx not found!"),TEXT("Error"),MB_ICONSTOP|MB_OK);
		exit(1);
	}

	len=filelength(hidx);
	gfx=(struct gfx_struct *)xmalloc(len);
	if (!gfx) {
		dd_deinit();
		save_options();
		MessageBox(desk_hwnd,TEXT("Error allocating menory!"),TEXT("Error"),MB_ICONSTOP|MB_OK);
		exit(1);
	}
	if (len<sizeof(struct gfx_struct)*40000) {
		dd_deinit();
		save_options();
		MessageBox(desk_hwnd,TEXT("Graphics file gx00.idx corrupt!"),TEXT("Error"),MB_ICONSTOP|MB_OK);
		exit(1);
	}
	read(hidx,gfx,len);
	close(hidx);
	...
	if (hidx==-1) {
		dd_deinit();
		save_options();
		MessageBox(desk_hwnd,TEXT("Graphics file gx00.dat not found!"),TEXT("Error"),MB_ICONSTOP|MB_OK);
		exit(1);
	}
}
...
void *conv_load(int nr,int *xs,int *ys)
{
	...
	if (gfx[nr].xs<0 || gfx[nr].xs>1000 || gfx[nr].ys<0 || gfx[nr].ys>1000) {
		dd_deinit();
		save_options();
		MessageBox(desk_hwnd,TEXT("Graphics file gx00.idx is corrupt!"),TEXT("Error"),MB_ICONSTOP|MB_OK);
		exit(1);
	}

	len=gfx[nr].xs*gfx[nr].ys*2;
	ptr=(unsigned short *)xmalloc(len);
	if (!ptr) {
		dd_deinit();
		save_options();
		MessageBox(desk_hwnd,TEXT("Error allocating memory!"),TEXT("Error"),MB_ICONSTOP|MB_OK);
		exit(1);
	}
...
int create_pnglib(HWND hwnd)
{
	...
	if (hidx==-1 || hpng==-1) {
		MessageBox(0,TEXT("creating files failed in create_pnglib()"),TEXT("oops"),MB_OK);
		return 0;
	}

	idx=(int *)calloc(MAXSPRITE,sizeof(int));
	...
	if(dir) {
		while(ent=readdir(dir)) {
			if(dir->info.attrib & _A_SUBDIR != _A_SUBDIR) {
				nr=atoi(ent->d_name);
				if(nr<0 || nr>=MAXSPRITE) continue;
				sprintf(buf,"STATUS: Preparing pnglib.dat: %d",nr);
		#ifdef _UNICODE
				...
				hin=open(ent->d_name,O_RDONLY|O_BINARY);
				if (hin==-1) continue;
...
int init_pnglib(void)
{
	...
	if (hidx==-1) {
		dd_deinit();
		save_options();
		MessageBox(desk_hwnd,TEXT("Graphics file pnglib.idx not found!"),TEXT("Error"),MB_ICONSTOP|MB_OK);
		exit(1);
	}
	...
	if (fpng==NULL) {
		dd_deinit();
		save_options();
		MessageBox(desk_hwnd,TEXT("Graphics file pnglib.dat not found!"),TEXT("Error"),MB_ICONSTOP|MB_OK);
		exit(1);
	}
