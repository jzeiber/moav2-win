...
void *xmalloc(int size)
{
	...
	ptr=(char *)HeapAlloc(heap,0,size);
...
void *xcalloc(int size1,int size2)
{
	...
	ptr=(unsigned char *)HeapAlloc(heap,HEAP_ZERO_MEMORY,size);
	usedmem+=HeapSize(heap,0,ptr)+16;
...
void dd_clear(void)
{
	...
	ptr=(unsigned short *)dd_get_ptr(sur1);
	if (!ptr) return;
	if (ptr) for (n=0; n<MAXX1*MAXY1; n++) ptr[n]=0;
	dd_release_ptr(sur1);

	ptr=(unsigned short *)dd_get_ptr(sur2);
	if (!ptr) return;
	if (ptr) for (n=0; n<MAXX1*MAXY1; n++) ptr[n]=0;
	dd_release_ptr(sur2);

	ptr=(unsigned short *)dd_get_ptr(suro);
	if (!ptr) return;
...
int dd_init_fullscreen(HWND hwnd,int x,int y)
{
...
void dd_deinit_fullscreen(void)
{
	if (sur1) {
...
int dd_init_windowed(HWND hwnd,int x,int y)
{
	...
	for (n=0; n<sizeof(vtab)/sizeof(struct vtab); n++) {
		...
		//surface.ddpfPixelFormat.u4.dwBBitMask=0x001f;
		surface.ddpfPixelFormat.u2.dwRBitMask=RED;
		surface.ddpfPixelFormat.u3.dwGBitMask=GREEN;
		surface.ddpfPixelFormat.u4.dwBBitMask=BLUE;
		surface.ddpfPixelFormat.u5.dwRGBAlphaBitMask=0;
...
{
	if(windowed==0) {
		return dd_init_fullscreen(hwnd,x,y);
	} else {
		return dd_init_windowed(hwnd,x,y);
	}
}
...
void dd_flip_windowed(void)
{
	...
	GetClientRect(desk_hwnd,&rect);

	// translate from client coords to screen coords
	MapWindowPoints(desk_hwnd,NULL,(LPPOINT)(&rect),2);
...
void dd_init_sprites(void)
{
	...
	if(memstat.dwTotalPhys > INT_MAX) {
		maxmem=INT_MAX;
	} else {
...
unsigned short *dd_load_png(FILE *fp,int *xs,int *ys,unsigned char **alpha_ptr,int *alphacnt_ptr)
{
    ...
    bmp=(unsigned short *)xmalloc(x*y*2);
	...
	for (m=0; m<x; m++) {
		...
		if (alphacnt>=alphamax-8) {
		    alphamax+=32;
		    alpha=(unsigned char *)realloc(alpha,alphamax);
		}
